...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE, $facebook = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ',username, email, id, password, active, last_login,bio,photo')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE || $facebook === TRUE)
			{
				if ($user->active == 0)
				{
					$this->trigger_events('post_login_unsuccessful');
     ...
     *
     * @return array
     * @author Wiktor Morawski
     **/
    public function get_user_highest_group($id=FALSE)
    {
        $this->trigger_events('get_user_highest_group');

        // if no id was passed use the current users id
        $id || $id = $this->session->userdata('user_id');

        return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description,'.$this->tables['groups'].'.priority')
            ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
            ->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
            ->limit(1)
            ->order_by($this->tables['groups'].'.priority','DESC')
            ->get($this->tables['users_groups']);
    }
